...
if (!evento || !partido) {
  alert("Error: evento no definido");
}

const carpeta = `eventos/${evento}/${partido}`;

const cloudName = "clickcontenidos";

function getThumbURL(carpeta, nombre){
  return `https://res.cloudinary.com/${cloudName}/image/upload/w_400,q_auto,f_auto/${carpeta}/${nombre}`;
}

function getFullURL(carpeta, nombre){
  return `https://res.cloudinary.com/${cloudName}/image/upload/w_1000,q_auto,f_auto/${carpeta}/${nombre}`;
}

function getZoomURL(carpeta, nombre){
...
img.src = getThumbURL(carpeta, nombre);
img.loading = "lazy";
...
imgGrande.src = getFullURL(carpeta, nombre);
...
img.src = getZoomURL(carpeta, `IMG_${num}.jpg`);
...
window.location.href = `carrito.html?evento=${evento}&partido=${partido}`;
